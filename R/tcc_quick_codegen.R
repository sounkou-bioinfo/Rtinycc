...
tcc_quick_codegen <- function(ir, decl, fn_name = "tcc_quick_entry") {
  ...
  if (identical(ir$tag, "rf_lang_call")) {
    ...
    c_args <- paste(sprintf("SEXP %s", arg_list), collapse = ", ")
    call_ctor <- switch(
      as.character(length(ir$args)),
      `1` = sprintf("Rf_lang2(Rf_install(\"%s\"), %s)", ir$fun, ir$args[[1]]),
      `2` = sprintf(
        "Rf_lang3(Rf_install(\"%s\"), %s, %s)",
        ir$fun,
        ir$args[[1]],
        ir$args[[2]]
      ),
      `3` = sprintf(
        "Rf_lang4(Rf_install(\"%s\"), %s, %s, %s)",
        ir$fun,
        ir$args[[1]],
        ir$args[[2]],
        ir$args[[3]]
  ...
  if (!identical(ir$tag, "scalar_expr")) {
    stop(
      "Codegen only supports nested_loop_kernel/rolling_mean_kernel/scalar_expr/rf_lang_call/internal_call2/switch_num_expr IR in current subset",
      call. = FALSE
    )
  }
