...
new_tccq_walker <- function(class = character(0), state = new.env(parent = emptyenv())) {
    structure(
        list(state = state),
...
tccq_visit_subset <- function(walker, node) {
    if (is.call(node) && length(node) == 3L && is.symbol(node[[1]]) && identical(as.character(node[[1]]), "[") && is.symbol(node[[2]])) {
        walker$state$arrays <- unique(c(walker$state$arrays, as.character(node[[2]])))
    }
    tccq_visit_base(walker, node)
}
