...
ffi_return_rule("cstring", value_expr) %as% {
  sprintf(
    paste(
      "if (%s) {",
      "    SEXP out = PROTECT(Rf_ScalarString(Rf_mkCharCE(%s, CE_UTF8)));",
      "    UNPROTECT(1);",
      "    return out;",
      "} else {",
      "    return R_NilValue;",
...
struct_field_getter_rule("cstring", field_name) %as% {
  c(
    ...
    sprintf("    SEXP out = PROTECT(Rf_ScalarString(Rf_mkCharCE(p->%s, CE_UTF8)));", field_name),
    "    UNPROTECT(1);",
    "    return out;",
    "  } else {",
    "    return R_NilValue;",
...
struct_field_setter_rule("cstring", field_name, size) %as% {
  if (!is.null(size)) {
    return(c(
      "  const char *src = Rf_translateCharUTF8(STRING_ELT(val, 0));",
      sprintf("  strncpy(p->%s, src, %d);", field_name, size - 1),
      sprintf("  p->%s[%d] = '\\0';", field_name, size - 1)
    ))
  }
  sprintf("p->%s = Rf_translateCharUTF8(STRING_ELT(val, 0));", field_name)
}
...
sexp_constructor_call_rule("cstring", arg_expr) %as% {
  sprintf("(%s ? Rf_ScalarString(Rf_mkCharCE(%s, CE_UTF8)) : R_NilValue)", arg_expr, arg_expr)
}
