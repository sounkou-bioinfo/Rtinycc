...
[![r2u-test](https://github.com/sounkou-bioinfo/Rtinycc/actions/workflows/r2u-test.yml/badge.svg)](https://github.com/sounkou-bioinfo/Rtinycc/actions/workflows/r2u-test.yml)
...
**Requirements**: R >= 4.4.0 (see [TESTING.md](TESTING.md) for r2u installation)

## Abstract

Rtinycc is an R interface to [TinyCC](https://github.com/TinyCC/tinycc), providing both CLI access and a libtcc-backed in-memory compiler. It includes an experimental FFI inspired by [Bun's FFI](https://bun.com/docs/runtime/ffi) for binding C symbols with predictable type conversions and pointer utilities. The package runs on unix-alikes and windows( pace UCRT issues, so WSL2 is recommended) and focuses on embedding TinyCC and enabling JIT-compiled bindings directly from R. Combined with [treesitter.c](https://github.com/sounkou-bioinfo/treesitter.c), which provides C header parsers, it can be used to rapidly generate declarative bindings.
...
`tcc_quick()` is the experimental R-to-C transpiler path in Rtinycc. It compiles a
declared subset of R code into C and executes it via TinyCC, while preserving a
safe fallback route to R evaluation through `Rf_lang*` + `Rf_eval` when needed.

**Important Limitation**: The current implementation uses pattern-matching "kernel matchers" 
that are hardcoded to recognize specific loop structures (nested loops, rolling windows). 
These matchers are brittle - adding intermediate variables or reordering statements can 
cause compilation to fall back to the R interpreter, even if the code is semantically 
equivalent. See the verification section below for details on formal checking.

### Convolution benchmark

This example benchmarks the classic convolution routine written in plain C (no manual
`SEXP` code). Rtinycc generates the `.Call` wrappers automatically. We compare base R,
`quickr`, `tcc_quick`, and a hand-written C FFI baseline.

```{r benchmark-tcc-quick-convolve}
library(quickr)
...
`quickr::quick()` and `tcc_quick()` to track progress on the same constructs.

```{r benchmark-tcc-quick-roll-mean}
...
### tcc_quick optimization guide

`tcc_quick()` targets a restricted subset of R for fast compilation. Understanding what's supported and what falls back to R evaluation helps you write efficient compiled functions.

The current implementation supports scalar arithmetic operations (`+`, `-`, `*`, `/`, `^`, `%%`, `%/%`), comparisons (`<`, `<=`, `>`, `>=`, `==`, `!=`), and logical operators (`&&`, `||`, `&`, `|`, `!`). Conditionals work through `if (cond) yes else no` or `ifelse(cond, yes, no)`, and numeric switch expressions are supported with `switch(sel, case1, case2, ...)`. Common math functions like `abs`, `sqrt`, `sin`, `cos`, `tan`, `asin`, `acos`, `atan`, `exp`, `log`, `log10`, `floor`, `ceiling`, `trunc`, and `tanh` are available. For more complex patterns, nested loops with array indexing using `for (i in seq_along(x))` and `x[i]` access are recognized, along with local scalar assignments like `a <- expr`.

Functions outside the supported subset can be handled through different fallback modes. The default `fallback = "auto"` falls back to the original R function when boundary calls like `.Call`, `.Internal`, `.Primitive`, or unsupported constructs are detected. Setting `fallback = "always"` returns the original function without attempting compilation, while `fallback = "never"` raises an error if the function is outside the supported subset. For calls that are not directly lowered (such as `max(x, y)` or `pmax(a, b, c)`), `tcc_quick` generates C code that builds a call pairlist and evaluates it via `Rf_eval()`, which preserves R's semantics but is slower than direct arithmetic.

To achieve optimal performance, use scalar `double`/`integer`/`logical` declarations where possible since mixed types incur conversion overhead. Minimize calls outside the core arithmetic/math subset as each fallback to `Rf_eval()` adds overhead. For loops, prefer nested `for (i in seq_along(x))` patterns that compile to indexed array kernels. The system memoizes based on function body and declarations, so repeated calls with the same function are fast. When debugging, use `debug = TRUE` to inspect generated C code and identify unexpected fallbacks, or use `mode = "code"` to preview generated C without compilation.

**Example: optimizing a function**

```r
# Slow: uses unsupported constructs
slow_fn <- function(x, y) {
  declare(type(x = double(1)), type(y = double(1)))
  result <- abs(x - y)
  if (result > 100) {
    log(result)  # fallback
  } else {
    sqrt(result)  # fallback
  }
}

# Fast: uses only supported constructs
fast_fn <- function(x, y) {
  declare(type(x = double(1)), type(y = double(1)))
  diff <- if (x > y) x - y else y - x  # inline abs
  if (diff > 100) {
    # log is supported
    log(diff)
  } else {
...
`tcc_quick()` includes a formal verification system (enabled by default with `verify = TRUE`) that performs comprehensive correctness checks. The system validates IR invariants to ensure all variables referenced in the intermediate representation are declared, types are consistent, and there's no use-before-definition. It also checks C code properties, verifying that generated C has balanced PROTECT/UNPROTECT calls, includes required headers, and validates bounds before allocation. Finally, it confirms codegen correctness by checking that function signatures match expectations and memory management is correct.

When verification detects issues, warnings are emitted but compilation proceeds. Use `verify = FALSE` to disable.

```r
# Enable verification (default)
fn_safe <- tcc_quick(my_function, verify = TRUE)

# Disable for production (not recommended)
fn_fast <- tcc_quick(my_function, verify = FALSE)
```

The verification system can also perform property-based testing:

```r
# Test semantic equivalence between R and compiled versions
source("R/tcc_quick_verify.R")
equivalence <- tcc_quick_test_equivalence(
  my_function,
  tcc_quick(my_function),
  tcc_quick_parse_declare(my_function),
  n_tests = 100
)
print(equivalence)
```

**Kernel matcher brittleness**: The current loop kernel matchers (`tcc_quick_try_lower_nested_loop_kernel`, `tcc_quick_try_lower_roll_mean_kernel`) are hardcoded pattern matchers, not general lowering. They require exact statement counts and structures. For example:

```r
# ✓ This compiles (matches kernel pattern)
...
# ✗ This falls back (added intermediate variable)
convolve_variant <- function(a, b) {
  declare(type(a = double(NA)), type(b = double(NA)))
...
This is a known limitation documented in `docs/rjit_critical_analysis.md`. Future work will replace kernel matchers with general statement-level lowering.
...
This work builds on [TinyCC](https://github.com/TinyCC/tinycc) and draws inspiration from [Bun's FFI](https://bun.com/docs/runtime/ffi), [CFFI](https://cffi.readthedocs.io/), [RSimpleFFI](https://github.com/sounkou-bioinfo/RSimpleFFI#readme), [CSlug](https://cslug.readthedocs.io/en/latest/), and [quickr](https://github.com/t-kalinowski/quickr).
