...
For thread-safe scheduling from worker threads, use `callback_async:<signature>` in `tcc_bind()`. The callback is enqueued from any thread and executed on the main R thread by the async dispatcher. Call `tcc_callback_async_enable()` once before use.
...
```{r ffi-sqlite-generated-bindings-check, eval=FALSE}
sqlite_header <- '
int sqlite3_exec(
  void *db,
  const char *sql,
