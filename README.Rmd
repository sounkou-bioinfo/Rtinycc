...
For thread-safe scheduling from worker threads, use `callback_async:<signature>` in `tcc_bind()`. Call `tcc_callback_async_enable()` once before use.

**Void return (fire-and-forget):** the callback is enqueued from any thread and executed on the main R thread automatically — on Windows via R's message pump, on Linux/macOS via R's event loop `addInputHandler`. No manual drain is required when R is idle at the console. `tcc_callback_async_drain()` is available for an explicit flush (useful in tests or batch scenarios).
...
For non-void return, the worker thread blocks until R returns the actual result:
...
struct itask { int (*cb)(void* ctx, int); void* ctx; int in; int out; };
static DWORD WINAPI iworker(LPVOID p) {
  struct itask* t = (struct itask*) p;
  t->out = t->cb(t->ctx, t->in);
  return 0;
}
int call_from_thread(int (*cb)(void* ctx, int), void* ctx, int x) {
  struct itask t; t.cb = cb; t.ctx = ctx; t.in = x; t.out = 0;
...
int call_from_thread(int (*cb)(void* ctx, int), void* ctx, int x) {
  struct itask t; t.cb = cb; t.ctx = ctx; t.in = x; t.out = 0;
  pthread_t th;
  pthread_create(&th, NULL, iworker, &t);
...
ffi_sync$call_from_thread(cb_triple, tcc_callback_ptr(cb_triple), 7L)  # returns 21
