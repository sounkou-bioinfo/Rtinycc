...
R functions can be registered as C function pointers via `tcc_callback()` and passed to compiled code. Provide a C signature when possible. Use `tcc_callback_ptr()` to obtain the pointer to pass into compiled code and `tcc_callback_close()` to release resources.
