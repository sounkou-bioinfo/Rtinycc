...
Two tiny cases are enough to make the main point:

- `noop()`: takes nothing and returns nothing
- `rand_unif(n)`: generates `n` random doubles

The `callme` version allocates the final R vector directly with the R C API. The
`Rtinycc` version returns a native `double*`, and the generated wrapper copies
that buffer into an R vector. That second case therefore includes the cost of
the copy.

```{r ffi-overhead, eval = has_callme}
rtinycc_code <- '
  #include <R.h>
  #include <Rinternals.h>
  #include <Rmath.h>
...
compile_times <- data.frame(
  ...
  milliseconds = round(c(
    ...
    median_elapsed(build_callme_module(), times = 2L)
  ) * 1000, 1)
)
...
noop_times <- data.frame(
  implementation = c("Rtinycc", "callme"),
  ns_per_call = round(c(
    median_elapsed(run_noop(rt_mod$noop, n_noop), times = 3L),
    median_elapsed(run_noop(cm_mod$noop, n_noop), times = 3L)
  ) / n_noop * 1e9, 1)
)

rand_times <- data.frame(
  implementation = c("Rtinycc", "callme"),
  us_per_call_n1 = round(c(
    median_elapsed(for (i in seq_len(5000L)) rt_mod$rand_unif(1L), times = 3L),
    median_elapsed(for (i in seq_len(5000L)) cm_mod$rand_unif(1L), times = 3L)
  ) / 5000L * 1e6, 1),
  us_per_call_n4096 = round(c(
    median_elapsed(for (i in seq_len(300L)) rt_mod$rand_unif(4096L), times = 3L),
    median_elapsed(for (i in seq_len(300L)) cm_mod$rand_unif(4096L), times = 3L)
  ) / 300L * 1e6, 1)
)
