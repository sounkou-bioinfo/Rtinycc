...
`tcc_quick` now emits native BLAS-backed paths for `%*%`, `crossprod`, and
`tcrossprod` (matrix/matrix cases) using `F77_CALL(dgemm)` through R headers.
This keeps behavior portable across platforms while still using R's linked BLAS
stack.
...
Allowlisted delegated operations (for example `solve` and `svd`) are still
...
In the example below, `%*%` and `crossprod` compile natively, while `solve`
continues to delegate to R.
