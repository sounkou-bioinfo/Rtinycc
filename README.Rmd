...
For treesitter-generated bindings, nested struct fields currently still fall
back to ptr-like accessors. The richer `struct:<name>` form is available when
you declare the nested field accessors explicitly.

Bitfields are a separate case: they use scalar helper accessors, but
`field_addr()` and `container_of()` reject bitfield members because bitfields do
not have ordinary addressable field semantics.

```{r bitfield-nested-note, eval = FALSE}
ffi <- tcc_ffi() |>
