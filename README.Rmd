...
Variadic C functions can be declared with `variadic = TRUE` and a typed `varargs = list(...)` tail in `tcc_bind()`. By default, calls are exact-tail (`varargs_min = length(varargs)`). To allow prefix calls up to a maximum typed tail, set `varargs_min` (for example `varargs_min = 0L` allows from zero up to all declared varargs).
...
We declare the fixed argument (`cstring` format) plus an explicit typed variadic tail.
With `varargs_min = 0L`, the binding accepts from 0 up to 2 extra typed arguments.
