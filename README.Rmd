...
building a shared library that registers `R_init_*` symbols. This is done by
casting the generated wrapper to the `.Call` ABI at runtime (via
...
`CC` config, there can still be subtle calling-convention or ABI mismatches
on some platforms or with unusual compiler flags. If you hit unexplained
crashes around `.Call`, treat this as a likely culprit.
