...
Variadic C functions can be declared with `variadic = TRUE` and a typed `varargs = list(...)` tail in `tcc_bind()`. In the current implementation, the variadic tail arity and types are explicit at bind-time (for example `varargs = list("i32", "f64")`), and calls must provide that full typed tail.
