...
- Comparisons: `<`, `<=`, `>`, `>=`, `==`, `!=`
- Logical operators: `&&`, `||`, `&`, `|`, `!`
- Conditionals: `if (cond) yes else no`, `ifelse(cond, yes, no)`
- Switch expressions: `switch(sel, case1, case2, ...)` (numeric selector only)
- Math functions: `abs`, `sqrt`, `sin`, `cos`, `tan`, `asin`, `acos`, `atan`, `exp`, `log`, `log10`, `floor`, `ceiling`, `trunc`, `tanh`
- Nested loops with array indexing: `for (i in seq_along(x))` with `x[i]` access
- Local scalar assignments: `a <- expr`

**Fallback behavior:**

Functions outside the supported subset can be handled three ways:

1. `fallback = "auto"` (default): Falls back to the original R function when boundary calls like `.Call`, `.Internal`, `.Primitive`, or unsupported constructs are detected.
2. `fallback = "always"`: Always returns the original function without attempting compilation.
3. `fallback = "never"`: Raises an error if the function is outside the supported subset.

For calls that are not directly lowered (e.g., `max(x, y)`, `pmax(a, b, c)`), `tcc_quick` generates C code that builds a call pairlist and evaluates it via `Rf_eval()`. This preserves R's semantics but is slower than direct arithmetic.

**Performance tips:**

- Use scalar `double`/`integer`/`logical` declarations where possible; mixed types incur conversion overhead.
- Minimize calls outside the core arithmetic/math subset. Each fallback to `Rf_eval()` adds overhead.
- For loops, prefer nested `for (i in seq_along(x))` patterns. These compile to indexed array kernels.
- Cache compiled functions; `tcc_quick()` memoizes based on function body and declarations, so repeated calls with the same function are fast.
- Use `debug = TRUE` to inspect generated C code and identify unexpected fallbacks.
- Use `mode = "code"` to preview generated C without compilation.
...
`tcc_quick()` includes a formal verification system (enabled by default with `verify = TRUE`) that checks:

1. **IR Invariants**: All variables referenced in the intermediate representation are declared, types are consistent, no use-before-definition
2. **C Code Properties**: Generated C has balanced PROTECT/UNPROTECT, includes required headers, validates bounds before allocation
3. **Codegen Correctness**: Function signatures match expectations, memory management is correct
