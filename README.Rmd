...
# Opt-in engine for examples that must execute on a remote machine. The chunk
# code is sent unchanged to `Rscript --vanilla -` over OpenSSH; no shell package
# or R package is required on the coordinating machine beyond knitr itself.
knitr::knit_engines$set(ssh = function(options) {
  if (!isTRUE(options$eval)) {
    return(knitr::engine_output(options, options$code, character()))
  }

  host <- if (is.null(options$host)) "rig" else options$host
  output <- system2(
    "ssh",
    c(host, "Rscript", "--vanilla", "-"),
    input = options$code,
    stdout = TRUE,
    stderr = TRUE
  )
...
The opt-in chunk uses the pure-R `ssh` knitr engine registered at the top of
this file. The complete displayed program is sent unchanged to
`Rscript --vanilla -` on the SSH host. Set `RTINYCC_RUN_SSH_DEMOS=true` to
execute it; ordinary CRAN and CI renders only show the code.

<details>
<summary>Show the complete Rtinycc, NVRTC, and CUDA program with live rig output</summary>

```{ssh cuda-nvrtc-rig, host="rig", eval=run_ssh_demos}
library(Rtinycc)
...
For a persistent asynchronous GPU queue, `mirai` supports launching daemons via
SSH directly. The explicit port is important here because `ssh_config()` uses
the port from its URL rather than the `Port` value of the `rig` SSH alias.
`crew` builds higher-level orchestration on mirai, but a dedicated mirai
`"gpu"` compute profile is the smaller mechanism for one rig.

```{r mirai-cuda-ssh, eval=FALSE}
library(mirai)
...
On the RTX 5050 rig this path produced an exact result for one million doubles
(`max_error = 0`). The measured call includes NVRTC compilation, CUDA context
creation, device allocation, transfers, launch, and synchronization; it is a
correctness demonstration rather than a steady-state kernel benchmark.
