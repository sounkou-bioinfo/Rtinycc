...
We use TinyCC in ABI mode (like CFFI) so the compiler performs the layout
computations (e.g. `sizeof`, `offsetof`, calling conventions) for the crossing,
which avoids a class of brittle, hand-rolled layout logic (think Python
ctypes-style calculations). This is similar in spirit to the libffi-based
approach in RSimpleFFI, but here the codegen uses TinyCC to do the crossing
directly instead of libffi’s call interface. For background and contrasts, see
the RSimpleFFI README: https://github.com/sounkou-bioinfo/RSimpleFFI#readme
