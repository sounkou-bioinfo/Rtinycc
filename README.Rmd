...
### Scalar/control-flow benchmark

This example benchmarks a scalar-heavy function with arithmetic, comparisons,
`ifelse()`, and `switch()`, showing the non-kernel transpilation path.
Because `quickr::quick()` currently does not support `switch()`, the quickr
path below uses an equivalent branch form.

```{r benchmark-tcc-quick-scalar}
slow_scalar <- function(x, y, k) {
  declare(type(x = double(1)), type(y = double(1)), type(k = integer(1)))
  z <- ifelse(x > y, x * y + sqrt(x), x - y / 2)
  switch(k,
         z + 1,
         z - 1,
         z * 2)
...
slow_scalar_quickr <- function(x, y, k) {
  declare(type(x = double(1)), type(y = double(1)), type(k = integer(1)))
  z <- ifelse(x > y, x * y + sqrt(x), x - y / 2)
  if (k == 1L) {
    z + 1
...
quick_scalar <- tcc_quick(slow_scalar, fallback = "never")
quickr_scalar <- tryCatch(
  quick(slow_scalar_quickr),
...
x <- runif(n)
y <- runif(n)
k <- sample.int(3, n, replace = TRUE)

scalar_exprs <- list(
  R = {
    out <- numeric(n)
    for (i in seq_len(n)) out[i] <- slow_scalar(x[i], y[i], k[i])
    out
  },
  Rtinycc_quick = {
    out <- numeric(n)
    for (i in seq_len(n)) out[i] <- quick_scalar(x[i], y[i], k[i])
    out
  }
)

if (!is.null(quickr_scalar)) {
  scalar_exprs$quickr <- {
    out <- numeric(n)
    for (i in seq_len(n)) out[i] <- quickr_scalar(x[i], y[i], k[i])
    out
  }
}

timings_scalar <- do.call(bench::mark, c(scalar_exprs, list(min_time = 1)))

if (is.null(quickr_scalar)) {
  message("quickr scalar example skipped in this run (unsupported construct)")
}

print(timings_scalar)
plot(timings_scalar, type = "boxplot") + bench::scale_x_bench_time(base = NULL)
```
