...
For low-level pointer-style calls,
[`tcc_call_symbol()`](https://sounkou-bioinfo.github.io/Rtinycc/reference/tcc_call_symbol.md)
can also use an R [`.C()`](https://rdrr.io/r/base/Foreign.html)-like
convention: call a `void` C routine with pointers to copied argument
buffers, then return a list of the modified values.
...
preserved R function. The legacy `threadsafe` argument is informational;
worker dispatch is selected by `callback_async:<signature>`.
...
conventional precompiled
[`.Call()`](https://rdrr.io/r/base/CallExternal.html) shared library.
