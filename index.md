...
#> <pointer: 0x5f8879a235c0>
tcc_data_ptr(ptr_ref)
#> <pointer: 0x5f8879c133c0>
...
- **Array fields in structs**: use
  `list(type = "u8", size = 16, array = TRUE)` and the generated helpers
  `struct_<name>_get_<field>()`, `struct_<name>_get_<field>_elt(i)`,
  `struct_<name>_set_<field>_elt(i, v)`.
...
.Internal(inspect(x))
#> @5f887b9f42b8 13 INTSXP g0c0 [REF(65535)]  1 : 100 (compact)
...
.Internal(inspect(x))
#> @5f887b9f42b8 13 INTSXP g0c0 [MARK,REF(65535)]  11 : 110 (expanded)
```
...
and passed to compiled code. Provide a C signature that matches the R
callback arguments and always call
[`tcc_callback_close()`](https://sounkou-bioinfo.github.io/Rtinycc/reference/tcc_callback_close.md)
when finished. When using
[`tcc_bind()`](https://sounkou-bioinfo.github.io/Rtinycc/reference/tcc_bind.md),
specify a `callback:<signature>` argument so the trampoline can be
generated, and pass `tcc_callback_ptr(cb)` as the user data pointer to
the C API.
...
to generate allocation and accessor helpers and
[`tcc_introspect()`](https://sounkou-bioinfo.github.io/Rtinycc/reference/tcc_introspect.md)
for size/alignment information. Free struct instances when you are done.
...
and exported as helper functions:
...
#### How all of this works (and why it is unusual)
...
runtime (via `.RtinyccCall`, which is just
[`base::.Call`](https://rdrr.io/r/base/CallExternal.html)). This is
...
We use TinyCC in ABI mode (like CFFI) so the compiler performs the
layout computations (e.g. `sizeof`, `offsetof`, calling conventions) for
the crossing, which avoids a class of brittle, hand-rolled layout logic
(think Python ctypes-style calculations). This is similar in spirit to
the libffi-based approach in RSimpleFFI, but here the codegen uses
TinyCC to do the crossing directly instead of libffi’s call interface.
For background and contrasts, see the RSimpleFFI README:
<https://github.com/sounkou-bioinfo/RSimpleFFI#readme>
...
(and other helpers that allocate) are tagged as owned and will be freed
by
[`tcc_free()`](https://sounkou-bioinfo.github.io/Rtinycc/reference/tcc_free.md)
or by their finalizer. Borrowed pointers (e.g. from
[`tcc_data_ptr()`](https://sounkou-bioinfo.github.io/Rtinycc/reference/tcc_data_ptr.md))
are tagged as borrowed and must not be freed by Rtinycc. Array returns
are copied into an R vector; use `free = TRUE` only when the C function
...
Use pointer utilities for pointer fields, and small wrappers for
by-value fields:
...
ffi$struct_outer_in_addr(o) |>
  tcc_ptr_set(in_ptr)
#> <pointer: 0x5f887b5bc980>
ffi$struct_outer_in_addr(o) |>
  tcc_data_ptr() |>
  (\(p) { ffi$struct_inner_set_a(p, 42L) })()
#> <pointer: 0x5f887a0519c0>
...
Name the type in
[`tcc_source()`](https://sounkou-bioinfo.github.io/Rtinycc/reference/tcc_source.md)
and bind it (still uses helpers to reach fields):
...
ffi <- tcc_ffi() |>
  ...
  tcc_struct("wrapper", accessors = c(anon = "ptr")) |>
  tcc_field_addr("wrapper", "anon") |>
  tcc_compile()

w <- ffi$struct_wrapper_new()
...
Use pointer utilities and raw byte helpers:

``` r
ffi <- tcc_ffi() |>
  tcc_source('struct buf { unsigned char data[16]; };') |>
  tcc_struct("buf", accessors = list(data = list(type = "u8", size = 16, array = TRUE))) |>
  tcc_compile()

b <- ffi$struct_buf_new()
ffi$struct_buf_set_data_elt(b, 0L, 255L)
#> <pointer: 0x5f887caa6c80>
