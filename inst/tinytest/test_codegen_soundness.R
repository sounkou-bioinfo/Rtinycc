...
expect_equal(ffi$copy_ints(input_ints, length(input_ints)), input_ints,
...
expect_equal(ffi$copy_doubles(input_dbls, length(input_dbls)), input_dbls,
...
expect_equal(ffi$copy_raw(input_raw, length(input_raw)), input_raw,
...
expect_equal(ffi$copy_logicals(input_lgl, length(input_lgl)), input_lgl,
...
ffi <- tcc_ffi() |>
  ...
  tcc_compile()
expect_true(is.null(ffi$return_null(5L)),
...
ffi <- tcc_ffi() |>
  ...
  ") |>
  tcc_struct("all_types", accessors = c(
    v_i8   = "i8",
    v_i16  = "i16",
    v_i32  = "i32",
    v_i64  = "i64",
...
expect_equal(ffi$call_int_cb(cb, cb_ptr, 21L), 42L,
...
expect_equal(ffi$call_bool_cb(cb, cb_ptr, TRUE), FALSE,
             info = "callback bool round-trip: !TRUE=FALSE")
expect_equal(ffi$call_bool_cb(cb, cb_ptr, FALSE), TRUE,
...
for (i in seq_len(100)) {
  ...
  r1 <- ffi$add_i32(i, -i)
  if (r1 != 0L) { gc_stress_ok <- FALSE; break }

  gc(verbose = FALSE)
  r2 <- ffi$add_f64(as.double(i), -as.double(i))
  if (r2 != 0.0) { gc_stress_ok <- FALSE; break }

  gc(verbose = FALSE)
  r3 <- ffi$echo_str(paste0("iter_", i))
  if (r3 != paste0("iter_", i)) { gc_stress_ok <- FALSE; break }

  gc(verbose = FALSE)
  r4 <- ffi$make_seq(5L)
  if (!identical(r4, 0:4)) { gc_stress_ok <- FALSE; break }
}
expect_true(gc_stress_ok, info = "GC stress: 100 iterations with forced GC survived")
...
r <- ffi$counted_i64()
expect_equal(ffi$get_scalar_calls(), 1L,
...
r <- ffi$counted_str()
expect_equal(ffi$get_str_calls(), 1L,
...
expect_equal(ffi$find_str(haystack, 4L, "gamma"), 2L,
             info = "cstring_array: finds element at index 2")
expect_equal(ffi$find_str(haystack, 4L, "missing"), -1L,
             info = "cstring_array: returns -1 for missing")
expect_equal(ffi$find_str(haystack, 4L, "alpha"), 0L,
