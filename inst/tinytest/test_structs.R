...
expect_true(
  {
    ...
    p1 <- compiled$point_new()
    p1 <- compiled$point_set_x(p1, 0.0)
    p1 <- compiled$point_set_y(p1, 0.0)
    p1 <- compiled$point_set_id(p1, 1L)

    p2 <- compiled$point_new()
    p2 <- compiled$point_set_x(p2, 3.0)
    p2 <- compiled$point_set_y(p2, 4.0)
    p2 <- compiled$point_set_id(p2, 2L)

    x1 <- compiled$point_get_x(p1)
    y2 <- compiled$point_get_y(p2)
    dist <- compiled$point_distance(p1, p2)

    compiled$point_free(p1)
    compiled$point_free(p2)
...
expect_true(
  {
    ...
    student <- compiled$student_new()
    student <- compiled$student_set_id(student, 42L)
    student <- compiled$student_set_grade(student, 3.14)

    id_ptr <- compiled$student_id_addr(student)
    student2 <- compiled$student_from_id(id_ptr)
    id2 <- compiled$student_get_id(student2)

    compiled$student_free(student)
...
expect_true(
  {
    ...
    size <- compiled$data_sizeof()
    align <- compiled$data_alignof()
