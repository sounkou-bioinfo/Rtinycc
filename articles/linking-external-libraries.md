...
from the system math runtime. For robustness, this vignette resolves an
actual runtime library file instead of relying on a short name. On Linux
that is typically a versioned `libm.so.*` file, while on macOS the
...
When given a full path, `Rtinycc` adds the containing directory and asks
TinyCC to link that exact file name. This matters for versioned runtime
libraries such as `libm.so.6`, which are not interchangeable with a
generic development symlink such as `libm.so` on every Linux
distribution. When given a short name, TinyCC resolves it through its
