...
`Rtinycc` converts R values to these C types inside generated wrappers. The
scalar integer and logical types are validated, not just cast silently.
...
The first four map directly onto R vectors. `character_array` passes string
objects, while `cstring_array` builds a temporary `const char **` for the call.
