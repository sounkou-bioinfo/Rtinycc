...
functions such as `sqrt()` and `cos()` are usually resolved from `libm`. For
robustness across distributions, this vignette resolves an actual shared-object
filename such as `libm.so.6` instead of relying on the unversioned `libm.so`
entry, which may be a linker script on Debian-like systems.

```{r, eval = !is_windows && !is.null(libm_path)}
math <- tcc_link(
  libm_path,
  symbols = list(
...
When given a full path, `Rtinycc` derives the containing directory and the
linker-facing library name automatically. When given a short name, TinyCC
resolves it through its configured library search paths.
