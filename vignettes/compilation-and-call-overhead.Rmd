...
rtinycc_code <- '
  ...
  double* rand_unif(int n) {
    ...
    if (n == 0) {
      /* The wrapper uses free = TRUE, so this zero-length buffer is copied
         as length 0 and then freed by the generated return path. */
...
```{r, eval = requireNamespace("callme", quietly = TRUE)}
...
```{r, eval = requireNamespace("callme", quietly = TRUE) && requireNamespace("bench", quietly = TRUE)}
